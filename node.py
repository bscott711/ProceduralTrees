...
class Node:
    ...
    def add_left(self, age) -> None:
        length = max(
            random.randint(cts.min_length, cts.max_length) - age, cts.min_length
        )
        angle = random.randint(cts.min_angle_left, cts.max_angle_left)
        self.left = Node(
            age * 2, length, angle, self.palette
        )  # Child nodes inherit the same palette
    ...
    def grow(self, age) -> None:
        ...
        if number == 1 and self.left == None:
            self.add_left(age)
        elif number == 2 and self.right == None:
            self.add_right(age)
        else:
...
def draw_parallel_lines(
    start, stop, perp_angle, width, palette, window
):  # Perp angle needs to be in radians https://www.desmos.com/calculator/hh236r60m7
...
def get_position(
    start: tuple[float, float], radius: float, angle: float
):  # Takes rectangular and polar coords and adds them
    return (
...
def draw_branches(
    node: Node, start: tuple[float, float], window: pygame.Surface
) -> None:  # Draws every node in a tree, along with connections
    if node == None:
        return
    ...
    pygame.draw.circle(
        window, node.palette["trunk0"], pos, width * 0.6
    )  # This brown circle colors in gaps between braches that are very bent
    draw_parallel_lines(
        start, pos, math.radians(node.angle + 90), width, node.palette, window
    )  # Draws the branch for each node
    ...
    draw_branches(
        node.left, pos, window
    )  # Recursively draws node's left and right children
...
def draw_leaves(
    ...
    if count(node) < cts.children_for_leaves:
        window.blit(
            node.leaves.surface, top_left_pos
        )  # If the node has few enough children draw leaves
    draw_leaves(
        node.left, pos, window
    )  # Recursively draws node's left and right children's leaves
...
def random_child(node: Node) -> Node:  # Picks a random node from the children of a root
    ...
    return random.choice(
        [valid for valid in choices if valid != None]
    )  # Only returns valid choices (Not None and having 2 children)
