...
def shadow(
    ...
    tuple[int, int], pygame.Surface
]:  # Returns the center of the shadow and the shadow
    mask = pygame.mask.from_surface(surface)
    shadow = mask.to_surface(setcolor=color, unsetcolor=(0, 0, 0, 0))
    return (
        mask.centroid(),
        pygame.transform.scale(
            shadow, (shadow.get_width(), shadow.get_height() / cts.leaves_shadow_ratio)
        ),
    )
